...
class Classes extends FooModel {
    ...
    public function getComments($class_id) {

        // Get class
        $params = array(
            'id' => $class_id,
        );
        $class = $this->selectItem($params);
        ...
        $class->cache_comments = json_encode($users_comments);
        $class->cache_time = time();
        $class->save();
    ...
    public function updateItem($params = [], $id = NULL) {
        ...
        //get class item by conditions
        $_params = [
            'id' => $id,
        ];
        $class = $this->selectItem($_params);
